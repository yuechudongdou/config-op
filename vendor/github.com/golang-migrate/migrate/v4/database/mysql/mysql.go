...
func (m *Mysql) Open(url string) (database.Driver, error) {
	...
	_, err = conn.ExecContext(context.Background(), fmt.Sprintf("CREATE DATABASE IF NOT EXISTS %s", destDB))
	if err != nil {
		return nil, err
	}

	config.DBName = destDB
	db, err = sql.Open("mysql", config.FormatDSN())
	if err != nil {
		return nil, err
	}
...
func (m *Mysql) Lock() error {
	return database.CasRestoreOnErr(&m.isLocked, false, true, database.ErrLocked, func() error {
		...
		lockID := fmt.Sprintf("%s:%s", m.config.DatabaseName, m.config.MigrationsTable)
		fmt.Println("start to lock the database; lockId: ", lockID)
		aid, err := database.GenerateAdvisoryLockId(lockID)
		if err != nil {
			fmt.Println("fail to get lock; lockId: ", lockID, "error: ", err)
			return err
		}
...
func (m *Mysql) Unlock() error {
	return database.CasRestoreOnErr(&m.isLocked, true, false, database.ErrNotLocked, func() error {
		...
		if _, err := m.conn.ExecContext(context.Background(), query, aid); err != nil {
			fmt.Println("fail to release lock; lockId: ", lockID, "error: ", err)
			return &database.Error{OrigErr: err, Query: []byte(query)}
		}
		fmt.Println("release lock for the database; lockId: ", lockID)
...
func (m *Mysql) Run(migration io.Reader) error {
	...
	query := string(migr[:])
	fmt.Println("start to exec the query")
	if _, err := m.conn.ExecContext(context.Background(), query); err != nil {
		fmt.Println("fail to exec the query")
		return database.Error{OrigErr: err, Err: "migration failed", Query: migr}
	}
	fmt.Println("finish to exec the query")
	return nil
}
